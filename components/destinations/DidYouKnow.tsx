...
import JournalDrawer, { type JournalFact } from "./JournalDrawer"

export type Fact = JournalFact
...
export default function DidYouKnow({
  ...
  useEffect(() => {
    if (facts.length <= 1 || isStoryOpen) return

    const timer = window.setTimeout(() => {
      setActiveIndex((currentIndex) => (currentIndex + 1) % facts.length)
    }, 5000)
